...
import { JLPTLevel, ContentType, GrammarLesson, QuizSession, Language } from "../types";
...
const grammarSchema: Schema = {
  ...
  properties: {
    ...
    introduction: { type: Type.STRING, description: "Brief intro to the topic" },
    points: {
      type: Type.ARRAY,
      items: {
        type: Type.OBJECT,
        properties: {
          pattern: { type: Type.STRING, description: "The grammar pattern (e.g., ～てはいけません)" },
          meaning: { type: Type.STRING, description: "Meaning in the user's language" },
          explanation: { type: Type.STRING, description: "Detailed explanation in the user's language" },
          examples: {
            type: Type.ARRAY,
            items: {
              type: Type.OBJECT,
              properties: {
                ...
                translation: { type: Type.STRING, description: "Translation in the user's language" },
              },
  ...
  properties: {
    ...
    questions: {
      ...
      items: {
        type: Type.OBJECT,
        properties: {
          question: { type: Type.STRING, description: "The question text (Japanese)" },
          options: {
            type: Type.ARRAY,
          ...
          explanation: { type: Type.STRING, description: "Why the answer is correct (in the user's language)" },
        },
...
const getLanguageName = (lang: Language): string => {
  switch (lang) {
    case Language.ZH_CN: return "Simplified Chinese";
    case Language.ZH_TW: return "Traditional Chinese";
    case Language.JA: return "Japanese";
    case Language.EN: default: return "English";
  }
};
