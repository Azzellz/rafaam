...
export const ConversationView: React.FC<Props> = ({ data, language, onExit }) => {
  ...
  const connect = async () => {
    ...
    try {
      ...
      const langName = language === Language.JA ? "Japanese" : 
                      language === Language.ZH_CN ? "Simplified Chinese" : 
                      language === Language.ZH_TW ? "Traditional Chinese" : "English";
  ...
  const handleServerMessage = async (message: LiveServerMessage) => {
    ...
    const flushUserInput = () => {
       ...
       setHistory(prev => {
         ...
         if (last?.role === 'user') {
            const needsSpace = !/[\u3000-\u303f\u3040-\u309f\u30a0-\u30ff\uff00-\uff9f]/.test(last.text.slice(-1));
            return [...prev.slice(0, -1), { ...last, text: last.text + (needsSpace ? ' ' : '') + text }];
         }
            ...
            <div className="space-y-3 font-['DotGothic16']">
               {history.map((msg, i) => (
                  <div key={i} className={`flex flex-col ${msg.role === 'user' ? 'items-end' : 'items-start'}`}>
                     <span className="text-xs text-gray-500 mb-1 font-['VT323'] tracking-wider">
                       {msg.role === 'user' ? t.transcriptUser : t.transcriptModel}
                     </span>
                     <div className={`px-3 py-2 text-sm md:text-base border-2 ${msg.role === 'user' ? 'bg-[#dbeafe] border-[#1e40af] text-[#1e3a8a] rounded-tr-none' : 'bg-white border-black rounded-tl-none'}`}>
                        {msg.text}
                     </div>
                  </div>
               ))}
               ...
               {currentInput && (
                  <div className="flex flex-col items-end opacity-80">
                     <span className="text-xs text-gray-500 mb-1 font-['VT323']">{t.transcriptUser}</span>
               ...
               {currentOutput && (
                  <div className="flex flex-col items-start opacity-80">
                     <span className="text-xs text-gray-500 mb-1 font-['VT323']">{t.transcriptModel}</span>
